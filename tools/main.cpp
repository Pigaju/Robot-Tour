...
static const int NUM_MENU_ITEMS = 12;
...
// Anti-stall jitter: if robot hasn't moved for 2.5s, apply a brief PWM burst
static unsigned long bfs_last_motion_ms = 0;       // last time encoders showed movement
static int32_t bfs_stall_enc_L = 0;                // encoder snapshot for stall detection
static int32_t bfs_stall_enc_R = 0;
static unsigned long bfs_jitter_start_ms = 0;      // when current jitter burst started
#define BFS_STALL_DETECT_MS 2500    // detect stall after this long with no encoder change
#define BFS_JITTER_DURATION_MS 150  // jitter burst duration
#define BFS_JITTER_PWM 160          // PWM for jitter burst
...
#define BFS_DRIVE_STEER_KI 2.0f
...
// Wheel test phase — declared early so enterScreen() can reference it.
enum WheelTestPhase { WT_IDLE, WT_RUNNING, WT_DONE };
static WheelTestPhase wt_phase = WT_IDLE;
...
static void enterScreen(ScreenState next) {
  ...
  // Reset wheel test to idle on entry
  if (next == SCREEN_WHEEL_TEST) {
    wt_phase = WT_IDLE;
    readWheelEncodersForce();
  }
...
// ---- WHEEL TEST: spin both wheels at full PWM and compare encoders ----
static float wt_duration_s = 2.0f;        // configurable via dial
static unsigned long wt_start_ms = 0;
static int32_t wt_enc_start_L = 0;
static int32_t wt_enc_start_R = 0;
static int32_t wt_enc_end_L = 0;
static int32_t wt_enc_end_R = 0;
static float wt_result_distL = 0.0f;
static float wt_result_distR = 0.0f;
static float wt_result_diff_pct = 0.0f;

static void wheelTestStart() {
  readWheelEncodersForce();
  wt_enc_start_L = encoderL_count;
  wt_enc_start_R = encoderR_count;
  wt_start_ms = millis();
  wt_phase = WT_RUNNING;
  Serial.printf("WHEEL_TEST: START duration=%.1fs pwm=255\n", (double)wt_duration_s);
}

static void wheelTestTick() {
  if (wt_phase != WT_RUNNING) return;
  unsigned long elapsed = millis() - wt_start_ms;
  if (elapsed < (unsigned long)(wt_duration_s * 1000.0f)) {
    // Drive both wheels at full speed
    setMotorsForwardPwm(255, 255);
    applyMotorOutputs();
  } else {
    // Time's up — stop and record results
    stopAllMotors();
    applyMotorOutputs();
    readWheelEncodersForce();
    wt_enc_end_L = encoderL_count;
    wt_enc_end_R = encoderR_count;
    const int32_t dL = wt_enc_end_L - wt_enc_start_L;
    const int32_t dR = wt_enc_end_R - wt_enc_start_R;
    const float dLcorr = INVERT_LEFT_ENCODER_COUNT  ? -(float)dL : (float)dL;
    const float dRcorr = INVERT_RIGHT_ENCODER_COUNT ? -(float)dR : (float)dR;
    wt_result_distL = (pulsesPerMeterL > 1.0f) ? (dLcorr / pulsesPerMeterL) : 0.0f;
    wt_result_distR = (pulsesPerMeterR > 1.0f) ? (dRcorr / pulsesPerMeterR) : 0.0f;
    const float avg = 0.5f * (fabsf(wt_result_distL) + fabsf(wt_result_distR));
    wt_result_diff_pct = (avg > 0.001f)
        ? 100.0f * (wt_result_distL - wt_result_distR) / avg
        : 0.0f;
    wt_phase = WT_DONE;
    Serial.printf("WHEEL_TEST: DONE  L_pulses=%ld R_pulses=%ld  L_m=%.4f R_m=%.4f  diff=%.1f%%\n",
                  (long)dL, (long)dR,
                  (double)wt_result_distL, (double)wt_result_distR,
                  (double)wt_result_diff_pct);
  }
}

static void drawWheelTestScreen() {
  ui.fillScreen(TFT_BLACK);
  ui.setTextColor(TFT_DARKGREY);
  ui.setTextSize(1);
  ui.drawString(FW_VERSION, 120, 2);
  ...
  ui.setTextColor(TFT_CYAN);
  ui.setTextSize(2);
  ui.drawString("WHEEL TEST", 120, 25);

  ui.setTextColor(TFT_WHITE);
  ui.setTextSize(1);
  ui.drawString("Both wheels @ PWM 255", 120, 50);

  // Show duration setting
  char durBuf[24];
  snprintf(durBuf, sizeof(durBuf), "Duration: %.1fs", (double)wt_duration_s);
  ui.setTextColor(TFT_YELLOW);
  ui.drawString(durBuf, 120, 65);

  if (wt_phase == WT_RUNNING) {
    unsigned long elapsed = millis() - wt_start_ms;
    float remaining = wt_duration_s - (float)elapsed / 1000.0f;
    if (remaining < 0.0f) remaining = 0.0f;

    // Show live encoder counts
    readWheelEncoders();
    const int32_t dL = encoderL_count - wt_enc_start_L;
    const int32_t dR = encoderR_count - wt_enc_start_R;

    ui.setTextColor(TFT_RED);
    ui.setTextSize(2);
    char runBuf[24];
    snprintf(runBuf, sizeof(runBuf), "RUNNING %.1fs", (double)remaining);
    ui.drawString(runBuf, 120, 90);

    ui.setTextColor(TFT_WHITE);
    ui.setTextSize(1);
    char lBuf[32], rBuf[32];
    snprintf(lBuf, sizeof(lBuf), "L pulses: %ld", (long)dL);
    snprintf(rBuf, sizeof(rBuf), "R pulses: %ld", (long)dR);
    ui.drawString(lBuf, 120, 120);
    ui.drawString(rBuf, 120, 135);
  } else if (wt_phase == WT_DONE) {
    ui.setTextColor(TFT_GREEN);
    ui.setTextSize(2);
    ui.drawString("DONE", 120, 90);

    ui.setTextSize(1);
    ui.setTextColor(TFT_WHITE);
    char lBuf[40], rBuf[40], dBuf[40], plBuf[40], prBuf[40];
    snprintf(plBuf, sizeof(plBuf), "L pulses: %ld", (long)(wt_enc_end_L - wt_enc_start_L));
    snprintf(prBuf, sizeof(prBuf), "R pulses: %ld", (long)(wt_enc_end_R - wt_enc_start_R));
    snprintf(lBuf, sizeof(lBuf), "L dist: %.4f m", (double)wt_result_distL);
    snprintf(rBuf, sizeof(rBuf), "R dist: %.4f m", (double)wt_result_distR);
    snprintf(dBuf, sizeof(dBuf), "Diff: %.1f%%", (double)wt_result_diff_pct);
    ui.drawString(plBuf, 120, 115);
    ui.drawString(prBuf, 120, 130);
    ui.drawString(lBuf, 120, 150);
    ui.drawString(rBuf, 120, 165);

    // Highlight diff in color
    if (fabsf(wt_result_diff_pct) > 5.0f) {
      ui.setTextColor(TFT_RED);
    } else if (fabsf(wt_result_diff_pct) > 2.0f) {
      ui.setTextColor(TFT_YELLOW);
    } else {
      ui.setTextColor(TFT_GREEN);
    }
    ui.setTextSize(2);
    ui.drawString(dBuf, 120, 185);
  } else {
    // IDLE
    ui.setTextColor(TFT_GREEN);
    ui.setTextSize(2);
    ui.drawString("READY", 120, 100);
    ui.setTextColor(TFT_WHITE);
    ui.setTextSize(1);
    ui.drawString("Dial: adjust duration", 120, 130);
  }

  ui.setTextColor(TFT_YELLOW);
  ui.setTextSize(1);
  ui.drawString("Click: START/STOP", 120, 215);
  ui.drawString("Hold: EXIT", 120, 230);
}
  ...
  switch (sq_phase) {
    ...
    case SQ_TURN: {
      ...
      if (fabsf(yaw_err) < SQ_TURN_TOLERANCE) {
        ...
        // Reset integral on zero-crossing to prevent overshoot-driven jitter
        if ((yaw_err > 0.0f) != (sq_turn_prev_err > 0.0f) && sq_turn_prev_err != 0.0f) {
          sq_turnI = 0.0f;
        }
        ...
        // I and D corrections projected onto error direction.
        // sq_turnI and sq_turn_dFilt are signed; project onto current error sign
        // so they boost PWM when aligned with error, reduce when opposing.
        float errSign = (yaw_err > 0.0f) ? 1.0f : -1.0f;
        float correction = (sqTurnKi * sq_turnI + sqTurnKd * sq_turn_dFilt) * errSign;
        // Clamp correction so it can only adjust magnitude, not flip direction
  ...
  switch (str_phase) {
    ...
    case STR_DRIVING: {
      ...
      if (str_driven_m >= str_target_m - 0.08f) {
        ...
        const float str_steerKi = 8.0f;
        const float str_steerKd = 0.10f;
        ...
        // Positive steerCorr → yaw_err>0 → need to turn left → slow left, speed right
        float leftPwm = basePwm - steerCorr + (float)motor_bias_pwm;
        float rightPwm = basePwm + steerCorr - (float)motor_bias_pwm;
        if (leftPwm < 0.0f) leftPwm = 0.0f;
        if (rightPwm < 0.0f) rightPwm = 0.0f;
...
void loop() {
  ...
  if (currentScreen == SCREEN_MAIN_MENU) {
    ...
    if (bfs_run_phase == BFS_PHASE_IDLE && !bfs_run_initialized) {
      ...
      // Start run logging so we get SPIFFS logs for BFS/manual-waypoint runs.
      strncpy(dbg_lastStartPath, "BFS_NAV", sizeof(dbg_lastStartPath));
      dbg_lastStartPath[sizeof(dbg_lastStartPath) - 1] = 0;
      startRun();
    ...
    if (bfs_run_phase == BFS_PHASE_DONE) {
      // Save logs once when run finishes, then just hold motors off.
      if (runActive) {
        lastRunStopReason = "BFS_DONE";
        stopRun();
      } else {
        stopAllMotors();
      }
      ...
      if (bfs_state.path_index >= bfs_state.path_length - 1) {
        ...
        // Init anti-stall detection for this turn
        bfs_last_motion_ms = now;
        bfs_stall_enc_L = encoderL_count;
        bfs_stall_enc_R = encoderR_count;
        bfs_jitter_start_ms = 0;
      ...
      if (fabsf(yaw_err) < BFS_TURN_TOLERANCE_DEG) {
        ...
        // Reset integral on direction reversal (overshoot detection)
        if (bfs_turn_prev_err != 0.0f &&
            ((yaw_err > 0.0f && bfs_turn_prev_err < 0.0f) ||
             (yaw_err < 0.0f && bfs_turn_prev_err > 0.0f))) {
          bfs_turn_integral = 0.0f;
        }
        // Anti-windup: clamp integral contribution to ±20 PWM
        const float turnIMaxPwm = 20.0f;
        ...
        if (absErr > 45.0f) {
          ...
          basePwm = 115.0f + (absErr - 15.0f) * (BFS_TURN_PWM - 115.0f) / 30.0f;
        } else if (absErr > 5.0f) {
          // Crawl zone: ramp from 120 down to 115 between 15° and 5°
          float t = (absErr - 5.0f) / 10.0f;
          basePwm = 115.0f + t * 5.0f;
        } else {
          // Fine zone: 115 ensures motors can overcome static friction
          basePwm = 115.0f;
        }
        ...
        // Anti-stall jitter: detect no encoder movement for 2.5s during turn
        bool turnJittering = false;
        {
          bool encMoved = (encoderL_count != bfs_stall_enc_L) || (encoderR_count != bfs_stall_enc_R);
          if (encMoved) {
            bfs_last_motion_ms = now;
            bfs_stall_enc_L = encoderL_count;
            bfs_stall_enc_R = encoderR_count;
          ...
          if (bfs_last_motion_ms > 0 && (now - bfs_last_motion_ms) >= BFS_STALL_DETECT_MS) {
            // Stalled! Apply jitter burst in the direction of error
            if (bfs_jitter_start_ms == 0) {
              bfs_jitter_start_ms = now;
              Serial.printf("BFS_TURN: STALL detected, jittering err=%.1f\n", (double)yaw_err);
            }
            if ((now - bfs_jitter_start_ms) < BFS_JITTER_DURATION_MS) {
              int jSign = (yaw_err > 0.0f) ? 1 : -1;
              setMotorsPivotPwm(jSign, BFS_JITTER_PWM);
              applyMotorOutputs();
              turnJittering = true;
            } else {
              // Jitter done — reset stall timer so it can re-trigger
              bfs_last_motion_ms = now;
              bfs_stall_enc_L = encoderL_count;
              bfs_stall_enc_R = encoderR_count;
              bfs_jitter_start_ms = 0;
        ...
        // Log sample for BFS TURN phase (10 Hz)
        {
          static unsigned long lastBfsTurnLogSample = 0;
          if ((now - lastBfsTurnLogSample) >= 100u) {
            lastBfsTurnLogSample = now;
            RunLogSample s;
            memset(&s, 0, sizeof(s));
            s.t_ms = (runStartMs > 0) ? (now - runStartMs) : 0;
            s.forwardM = bfs_run_total_driven_m;
            s.yawRelDeg = imu_yaw;
            s.yawErrDeg = yaw_err;
            s.targetYawDeg = bfs_run_target_yaw_deg;
            s.yawIntegral = bfs_turn_integral;
            s.gz_dps = imu_gz_dps;
            s.gz_raw_dps = imu_gz_raw_dps;
            s.gz_bias_dps = imu_gz_bias_dps;
            s.basePWM = turnPwmF;
            s.corrRaw = correction;
            s.canPhase = (uint8_t)BFS_PHASE_TURN;
            ...
            s.steerIntegral = bfs_turn_integral;
            s.motorBiasPwm = (float)motor_bias_pwm;
            {
              const int32_t dL = encoderL_count - runStartEncL;
              const int32_t dR = encoderR_count - runStartEncR;
              const float dLcorr = INVERT_LEFT_ENCODER_COUNT ? -(float)dL : (float)dL;
      ...
      if (timeOk && varSmall) {
        ...
        // Init anti-stall detection for this drive segment
        bfs_last_motion_ms = millis();
        bfs_stall_enc_L = encoderL_count;
        bfs_stall_enc_R = encoderR_count;
        bfs_jitter_start_ms = 0;
      ...
      if (bfs_run_driven_m >= bfs_run_segment_dist_m - BFS_ARRIVE_TOLERANCE_M) {
        ...
        // Anti-stall jitter: detect no encoder movement for 2.5s during drive
        {
          bool encMoved = (encoderL_count != bfs_stall_enc_L) || (encoderR_count != bfs_stall_enc_R);
          if (encMoved) {
            bfs_last_motion_ms = now;
          ...
          if (bfs_last_motion_ms > 0 && (now - bfs_last_motion_ms) >= BFS_STALL_DETECT_MS) {
            if (bfs_jitter_start_ms == 0) {
              bfs_jitter_start_ms = now;
              Serial.printf("BFS_DRIVE: STALL detected, jittering yawErr=%.1f\n", (double)yaw_err);
            }
            if ((now - bfs_jitter_start_ms) < BFS_JITTER_DURATION_MS) {
              // Jitter: alternate forward burst at higher PWM
              setMotorsForwardPwm(BFS_JITTER_PWM, BFS_JITTER_PWM);
              applyMotorOutputs();
            } else {
              bfs_last_motion_ms = now;
              bfs_stall_enc_L = encoderL_count;
        ...
        // Log sample for BFS DRIVE phase (10 Hz)
        {
          static unsigned long lastBfsDriveLogSample = 0;
          if ((now - lastBfsDriveLogSample) >= 100u) {
            lastBfsDriveLogSample = now;
            RunLogSample s;
            memset(&s, 0, sizeof(s));
            s.t_ms = (runStartMs > 0) ? (now - runStartMs) : 0;
            s.meters = bfs_run_driven_m;
            s.forwardM = bfs_run_total_driven_m + bfs_run_driven_m;
            s.yawRelDeg = imu_yaw;
            s.yawErrDeg = yaw_err;
            s.targetYawDeg = bfs_run_target_yaw_deg;
            s.yawIntegral = bfs_drive_integral;
            s.gz_dps = imu_gz_dps;
            s.gz_raw_dps = imu_gz_raw_dps;
            s.gz_bias_dps = imu_gz_bias_dps;
            s.basePWM = basePwm;
            s.corrRaw = steerCorr;
            s.corrFiltered = steerCorr;
            s.corrOut = steerCorr;
            s.canPhase = (uint8_t)BFS_PHASE_DRIVE;
            s.braking = (remaining < 0.20f) ? 1 : 0;
            s.lPwm = motor_l_speed;
            s.rPwm = motor_r_speed;
            s.mL_err_dir = motorL_err_dir;
            s.mL_err_pwm = motorL_err_pwm;
            ...
            s.regR_dir = motorR_last_reg_dir;
            s.regR_pwm = motorR_last_reg_pwm;
            s.remainingM = remaining;
            s.steerIntegral = bfs_drive_integral;
            s.motorBiasPwm = (float)motor_bias_pwm;
            {
              const int32_t dL = encoderL_count - runStartEncL;
              const int32_t dR = encoderR_count - runStartEncR;
  ...
  // ---- WHEEL TEST input ----
  else if (currentScreen == SCREEN_WHEEL_TEST) {
    if (wt_phase == WT_RUNNING) {
      wheelTestTick();
    }

    // Hold button to exit back to menu
    static bool wtLongPressHandled = false;
    if (!M5.BtnA.isPressed()) wtLongPressHandled = false;
    if (!wtLongPressHandled && M5.BtnA.pressedFor(800)) {
      wtLongPressHandled = true;
      suppressNextClick = true;
      stopAllMotors();
      applyMotorOutputs();
      wt_phase = WT_IDLE;
    ...
    if ((now - boot_ms) > 500 && M5.BtnA.wasClicked() && (now - last_button_ms) > 250) {
      last_button_ms = now;
      if (wt_phase == WT_IDLE || wt_phase == WT_DONE) {
        wheelTestStart();
      } else {
        stopAllMotors();
        applyMotorOutputs();
    ...
    // Dial adjusts duration when idle
    if (wt_phase == WT_IDLE && dialSteps != 0) {
      wt_duration_s += dialSteps * 0.5f;
      if (wt_duration_s < 0.5f) wt_duration_s = 0.5f;
      if (wt_duration_s > 10.0f) wt_duration_s = 10.0f;
    }
  }
